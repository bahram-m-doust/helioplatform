...
## 6c. External (`/v1/*`) public API for third-party sites

The agent microservices each expose a second router at `/v1/` aimed at
external callers (Framer landing pages, partner portals, etc.). The
internal `/api/*` routes used by the main-app frontend are unchanged.

**Routing** (handled by edge nginx):

```
https://api.helio.ae/v1/image/generate         -> image-generator-api
https://api.helio.ae/v1/video/generate         -> video-generator-api
https://api.helio.ae/v1/storyteller/chat       -> storyteller-api
https://api.helio.ae/v1/campaign/chat          -> campaign-maker-api
```

**Auth**: every request must carry `X-API-Key: <secret>`. Keys are
configured server-side via `HELIO_EXTERNAL_API_KEYS_<AGENT>` (comma-
separated, optionally `label:secret` pairs). Comparison is constant-time;
missing or wrong keys return `401`.

**Rate limiting**: two layers.

1. nginx `limit_req` per real client IP (60 r/m, burst 30) — stops
   abuse before it touches FastAPI.
2. FastAPI per-key token bucket (defaults: 30 r/m + burst 10; the
   video agent overrides to 10 r/m + burst 4 because renders are
   expensive). Returns `429` with `Retry-After`.

**CORS**: only origins listed in `HELIO_EXTERNAL_ALLOWED_ORIGINS` are
allowed for browser callers. Server-to-server callers ignore CORS.

**Bootstrap on a fresh deployment**:

```bash
# 1. Generate one secret per agent (keep them in a secrets manager).
for agent in image video storyteller campaign; do
  echo "$agent: helio_live_$(python -c 'import secrets; print(secrets.token_urlsafe(32))')"
done

# 2. Drop them into .env.prod:
HELIO_EXTERNAL_API_KEYS_IMAGE=helio_live_xxx
HELIO_EXTERNAL_API_KEYS_VIDEO=helio_live_yyy
HELIO_EXTERNAL_API_KEYS_STORYTELLER=helio_live_zzz
HELIO_EXTERNAL_API_KEYS_CAMPAIGN=helio_live_www
HELIO_EXTERNAL_ALLOWED_ORIGINS=https://yourstudio.framer.website,https://yourbrand.com

# 3. Reload only the affected services:
docker compose -f docker-compose.prod.yml --env-file .env.prod up -d \
  image-generator-api video-generator-api storyteller-api campaign-maker-api
```

**Quick verification** (replace `<KEY>`):
...
Per-agent docs (request/response shapes, error table, examples) live
inside each agent under `app/external/README.md`.
