"""Security primitives for the external image-generator API.

Three concerns, kept in one file because they're all request-scoped and
none are large enough to warrant their own module:

  * ``api_key_principal`` — FastAPI dependency that validates ``X-API-Key``
    against the configured key set in constant time.
  * ``RateLimiter``      — in-memory token bucket per principal, enough for
    a single replica. Move to Redis if/when we scale horizontally.
  * ``require_caller``   — composite dependency: auth + rate-limit + headers.
"""
...
def _resolve_principal(presented: str, config: ExternalConfig) -> str | None:
    """Return the label of the matching key, or ``None`` if no key matches.

    Uses ``secrets.compare_digest`` against every configured secret so the
    runtime is independent of which (or how many) keys are configured —
    this prevents timing oracles that would otherwise reveal valid keys.
    """
...
def api_key_principal(
    ...
    """FastAPI dependency: 401 if the key is missing/invalid, else the label."""
