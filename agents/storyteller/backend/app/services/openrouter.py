"""OpenRouter chat client for the Storyteller agent.

Supports a primary model plus a list of fallback models, mirroring the
frontend behaviour: on a 429 with upstream/rate-limit markers we try the
next model in order.
"""
...
def get_openrouter_model() -> str:
    return (
        os.getenv('STORYTELLER_LLM_MODEL')
...
def get_openrouter_fallback_models() -> list[str]:
    raw = (
        os.getenv('STORYTELLER_FALLBACK_MODELS')
