...
const heliogramDir = resolve(rootDir, 'heliogram');
const serverDir = resolve(heliogramDir, 'server');
const clientDir = resolve(heliogramDir, 'client');
...
function ensureCommunityEnv() {
  ...
  if (existsSync(envExamplePath)) {
    ...
    console.log('[community] heliogram/.env created from .env.example');
    return;
  }

  console.warn('[community] heliogram/.env not found and .env.example is missing.');
}
...
function runCompose(args, composeEnv) {
  return spawnSync(dockerCmd, ['compose', ...args], {
    cwd: heliogramDir,
    stdio: 'pipe',
    encoding: 'utf-8',
    env: composeEnv,
...
function startDetachedWithLogs(command, args, cwd, logPath) {
  ensureRuntimeDir();
  if (isWin) {
    const shouldUseCmdWrapper = /\.(cmd|bat)$/i.test(command);
    const spawnCommand = shouldUseCmdWrapper ? 'cmd.exe' : command;
    const spawnArgs = shouldUseCmdWrapper
      ? ['/d', '/s', '/c', [command, ...args].map((part) => toWindowsShellArg(part)).join(' ')]
      : args;

    // Windows: npm.cmd can fail with detached + stdio ignore unless wrapped by cmd.exe.
...
async function checkNativeDependencies(config) {
  ...
  if (!existsSync(resolve(clientDir, 'node_modules'))) {
    console.warn('[community] native dependency missing: heliogram/client/node_modules not found.');
    console.warn('[community] run: cd heliogram/client && npm install');
    return false;
  }
  ...
  if (!config.useSqlite) {
    ...
    if (config.dbHost === 'db') {
      ...
      console.warn('[community] set DB_HOST=127.0.0.1 (or PostgreSQL hostname) in heliogram/.env for native mode.');
      return false;
    }
...
async function startNative(config) {
  ...
  if (!backendHealthyBefore) {
    ...
    backendPid = startDetachedWithLogs(
      pythonCmd,
      ['manage.py', 'runserver', `127.0.0.1:${config.backendPort}`],
      serverDir,
      backendLogPath,
    );
  }
  ...
  if (!frontendHealthyBefore) {
    frontendPid = startDetachedWithLogs(
      npmCmd,
      ['run', 'dev', '--', '--host', '127.0.0.1', '--port', String(config.frontendPort), '--strictPort'],
      clientDir,
      frontendLogPath,
    );
  }
